...
float4x4 spotWVP;										// World * view * proj matrix for spotlight
float3 spotPosition;									// world coordinates of spotlight
texture spotTexture;									// light map texture
sampler2D spotMap = sampler_state						// sampler for light map
{
   Texture = (spotTexture);
   ADDRESSU = CLAMP;
   ADDRESSV = CLAMP;
...
//	Output.Fog = 1.0f - length(Output.ViewDirection) * 0.005f;										// linear fog
	...
	if (isSpotlit)
	{
		Output.SpotCoord = mul(Input.Position, spotWVP);											// Project vertex onto texture space	
		Output.SpotCoord.xy = Output.SpotCoord.xy / Output.SpotCoord.ww / 2.0f + 0.5f;				// cvt from homogeneous then from +/-1 to uv0-1
		Output.SpotCoord.z = clamp (1.0f - length(spotPosition - fvObjectPosition) * 0.01f, 0, 1);	// get a brightness value (1-0) from dist
	}
	else
	{
		Output.SpotCoord = 0;
...
{      
	...
	if (isSpotlit)
	{
		colour.rgb += tex2D(spotMap, Input.SpotCoord) * Input.SpotCoord.z;
	}
