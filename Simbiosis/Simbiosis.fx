...
Main_VS_OUTPUT Main_VS0( Main_VS_INPUT Input )
{
   Main_VS_OUTPUT Output;
	...
	if (isBump)
	{
		float3 fvBinormal       = normalize(mul( Input.Binormal, (float3x3)world ));		// normal is already in world space, now do same for tangent & binormal
		float3 fvTangent        = normalize(mul( Input.Tangent, (float3x3)world ));
		  
		Output.ViewDirection.x  = dot( fvTangent, fvViewDirection );						// rotate the view into tangent space
		Output.ViewDirection.y  = dot( fvBinormal, fvViewDirection );
		Output.ViewDirection.z  = dot( fvNormal, fvViewDirection );

		Output.LightDirection.x  = dot( fvTangent, fvLightDirection );						// rotate the sunlight into tangent space
...
{      
	...
	if (isBump)
	{
		// Fetch a normal from the normal map (this will be in tangent space)
...
//--------------------------------------------------------------//
// Main technique for creatures and terrain
// LOD2 - As main technique but single pass with no shadows
//--------------------------------------------------------------//
technique Main2
{
   pass Pass_0
